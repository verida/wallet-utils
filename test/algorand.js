...
describe("Algorand", function() {
    ...
    describe("Create Account", function() {
        ...
        it("Has valid address", function() {
            ...
            assert.ok(address == account.address, "Private key matches correct address")
        })

        it("Has valid message signing function", async function() {
            const message = "Do you approve access to view and update \"Verida Wallet\"?\n\n" + account['did'];
            
            const signature = await utils.signMessage('algo', account.privateKey, message)
            assert(signature && signature.length, true)

            const verifySignature = await utils.verifySignature('algo', message, signature, account.did)
